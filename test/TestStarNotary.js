...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
    let starId = 6;
    await instance.createStar('Star Six', starId, {from: accounts[0]});

    assert.equal(await instance.tokenIdToStarInfo.call(starId), 'Star Six');
    assert.equal(await instance.name(), "CJ Token");
    assert.equal(await instance.symbol(), "CJT");
});
